rs_histograma2<-function(x,y,k,bd){
	...
	mach_corpus <- Corpus(VectorSource(gids$text))
		...
		fc_stopwords<-function(dbs){
			switch(dbs,
				...
				jr_telcel = c("serviciotelcel",'serviciotelcel','servicio',"rt",'hola','gracias', stopwords("es")),
				jr_iusacell = c("movistar", "movistarmx","mimovistarmx","q","d","rt",'hola','gracias', stopwords("es")))
		}
